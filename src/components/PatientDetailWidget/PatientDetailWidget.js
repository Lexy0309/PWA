...
import * as SliderAPI from '../../googleslide';
...
class PatientDetailWidget extends Component {
	constructor (props) {
		...
		this.state = {
			...
			presentationCreattingDlg: false, presentationCreattingInstructionDlg: false, bCreatingPresentation: false
		}
	...
	handleSavePresentationInstructionDlg = () => {
		this.setState({ presentationCreattingInstructionDlg: true });
	}
	...
	handleSignoutClick() {
		// this should be done by firebase
		// firebase.auth().signOut();
		//window.gapi.auth2.getAuthInstance().signOut();
		firebase.auth().signOut();
	}

	addLogoToSlide = (objPageSize, presentationId, pageId, patientInfo) => {
		const SQUARE_SIZE = 1000000, OFFSET = 100000;
		const size = { width: SQUARE_SIZE, height: SQUARE_SIZE };
		const offset = { x: OFFSET, y: OFFSET }; // 'top-left'

		if ( this.state.presentationLogoSetting === 'top-right') {
			offset.x = objPageSize.width.magnitude - SQUARE_SIZE - OFFSET;
		} else if ( this.state.presentationLogoSetting === 'bottom-left') {
			offset.y = objPageSize.height.magnitude - SQUARE_SIZE - OFFSET;
		} else if ( this.state.presentationLogoSetting === 'bottom-right') {
			offset.x = objPageSize.width.magnitude - SQUARE_SIZE - OFFSET;
			offset.y = objPageSize.height.magnitude - SQUARE_SIZE - OFFSET;
		}
		if (patientInfo.avatar && patientInfo.avatar.startsWith('http')) {
			return SliderAPI.createImage(presentationId, pageId + '-logo', pageId, patientInfo.avatar, size, offset, 'EMU');
		} else {
			return [];
		}
	...
	addSubjectSlide = (result, patientInfo) => {
		const subject = 'Clinical Case';
		const author = 'By DoctorCases';
		let presentationId = result.presentationId;
		let objectID1 = result.slides[0].pageElements[0].objectId;
		let objectID2 = result.slides[0].pageElements[1].objectId;
		let pageId = result.slides[0].objectId;
		let req1 = SliderAPI.insertTextFromId(subject, presentationId, objectID1);
		let req2 = SliderAPI.insertTextFromId(author, presentationId, objectID2);
		let req3 = this.addLogoToSlide(result.pageSize, presentationId, pageId, patientInfo);
		let reqs = [...req1, ...req2, ...req3];
		return reqs;
	}

	addPatientDescriptionSlide = (result, pid, pages, patientInfo) => {
		let objPageSize = result.pageSize;
		let pageid = 'page-' + pages;
		let reqs = SliderAPI.appendSlide(pid, 'BLANK', pageid);
		// let eleLogo = this.addLogoToSlide(result.pageSize, pid, pageid);
		// reqs = [...reqs, ...eleLogo];
		// -------- Content ----------
		const DISTANCE_UNIT = objPageSize.width.unit;
		const LEFT_PANEL_WIDTH = objPageSize.width.magnitude * 0.4, RIGHT_PANEL_WIDTH = objPageSize.width.magnitude * 0.5;
		const AVATAR_HEIGHT = objPageSize.height.magnitude * 0.35;
		const OFFSET = objPageSize.width.magnitude * 0.025;
		// Create User Avatar
		/**
		 * Just Think about patientInfo.avatar
		 * sometimes it is a blob data not url link
		 * so over 2k bytes error occures
		 * */
		let eleUserAvatar = {}
		console.log('User Avatar :', patientInfo.avatar);
		if (patientInfo.avatar) {
			console.log('Avatar Inserting...');
			eleUserAvatar = SliderAPI.createImage(
				pid,
				pageid + '-patient-avatar',
				pageid,
				patientInfo.avatar,
				{ width: LEFT_PANEL_WIDTH, height: AVATAR_HEIGHT },
				{ x: OFFSET, y: OFFSET * 2 },
				'EMU'
			);
			reqs = [...reqs, ...eleUserAvatar];
		}
		// Create User Description
		let offset = { x: OFFSET, y: AVATAR_HEIGHT + OFFSET * 3 };
		let size = { width: LEFT_PANEL_WIDTH, height: objPageSize.height.magnitude - offset.y - OFFSET };
		let objOption = { size, offset, unit: DISTANCE_UNIT, bold: false, italic: false, fontSize: 18 }
		let strContent = "NAME : " + patientInfo.name + '\n' +
									"BIRTHDAY : " + patientInfo.birthday.toDateString() + '\n' +
									"BED : " + patientInfo.bed + '\n';
									// "EMAIL : " + patientInfo.email + '\n' +
									// "PHONE : " + patientInfo.phone;
		let eleUserDescription = SliderAPI.createTextboxWithText(pid, pageid, pageid + '-patient-description', strContent, objOption);

		// Create User Notes
		offset = { x: OFFSET * 3 + LEFT_PANEL_WIDTH, y: OFFSET };
		size = { width: RIGHT_PANEL_WIDTH, height: objPageSize.height.magnitude - offset.y - OFFSET };
		objOption = { size, offset, unit: DISTANCE_UNIT, bold: false, italic: false, fontSize: 10 }
		strContent = patientInfo.notes || 'No Evolution...';
		let eleUserNotes = SliderAPI.createTextboxWithText(pid, pageid, pageid + '-patient-notes', strContent, objOption);

		reqs = [...reqs, ...eleUserDescription, ...eleUserNotes];

		return reqs;
	}

	addPatientCaseImageSlides = (pid, pages, objPageSize, caseImageInfo) => {
		const DISTANCE_UNIT = objPageSize.width.unit;
		const IMAGE_SIZE_WIDTH = objPageSize.width.magnitude * 0.6, IMAGE_SIZE_HEIGHT = objPageSize.height.magnitude * 0.8;
		const TEXT_SIZE_WIDTH = objPageSize.width.magnitude * 0.3;
		const OFFSET = objPageSize.width.magnitude * 0.025;
				
		const strDescription =  caseImageInfo.description || 'No Description Created...';
		const strCaseDate = new Date(parseInt(caseImageInfo.case_date)).toDateString();
		const imageURL = caseImageInfo.thumburl || '';
		let pageid = 'page-' + pages;
		let reqs = SliderAPI.appendSlide(pid, 'BLANK', pageid);

		let size = { width: IMAGE_SIZE_WIDTH, height: IMAGE_SIZE_HEIGHT };
		let offset = { x: OFFSET, y: objPageSize.height.magnitude * 0.1 };
		let eleImage = SliderAPI.createImage(pid, pageid + '-image', pageid, imageURL, size, offset, DISTANCE_UNIT);
		const strDescriptionTitleId = pages + '-description-title';
		size = { width: TEXT_SIZE_WIDTH, height: objPageSize.height.magnitude * 0.1 };
		offset = { x: OFFSET * 3 + IMAGE_SIZE_WIDTH, y: objPageSize.height.magnitude * 0.1 };
		// strDate
		let objOption = { size, offset, unit: DISTANCE_UNIT, bold: true, italic: false, fontSize: 18 }
		let eleDescriptionTitle = SliderAPI.createTextboxWithText(pid, pageid, strDescriptionTitleId, strCaseDate, objOption);
		// strDescription
		const strDescriptionContentId = pages + '-description-content';
		offset = { x: OFFSET * 3 + IMAGE_SIZE_WIDTH, y: size.height + objPageSize.height.magnitude * 0.1 + 100000 };
		size = { width: TEXT_SIZE_WIDTH, height: objPageSize.height.magnitude - offset.y - objPageSize.height.magnitude * 0.1 };
		objOption = { size, offset, unit: DISTANCE_UNIT, bold: false, italic: true, fontSize: 14 }
		let eleDescriptionContent = SliderAPI.createTextboxWithText(pid, pageid, strDescriptionContentId, strDescription, objOption);
		reqs = [...reqs, ...eleImage, ...eleDescriptionTitle, ...eleDescriptionContent];
		return reqs;
	}

	addPatientCaseVideoSlides = (pid, pages, objPageSize, caseVideoInfo) => {
		const DISTANCE_UNIT = objPageSize.width.unit;
		const VIDEO_SIZE_WIDTH = objPageSize.width.magnitude * 0.6, VIDEO_SIZE_HEIGHT = objPageSize.height.magnitude * 0.8;
		const TEXT_SIZE_WIDTH = objPageSize.width.magnitude * 0.3;
		const OFFSET = objPageSize.width.magnitude * 0.025;
				
		const strDescription =  caseVideoInfo.description || 'No Description Created...';
		const strCaseDate = new Date(parseInt(caseVideoInfo.case_date)).toDateString();
		const videoURL = caseVideoInfo.thumburl || '';
		let pageid = 'page-' + pages;
		let reqs = SliderAPI.appendSlide(pid, 'BLANK', pageid);

		let size = { width: VIDEO_SIZE_WIDTH, height: VIDEO_SIZE_HEIGHT };
		let offset = { x: OFFSET, y: objPageSize.height.magnitude * 0.1 };
		let eleVideo = SliderAPI.createVideo(pid, pageid + '-video', pageid, videoURL, size, offset, DISTANCE_UNIT);
		const strDescriptionTitleId = pages + '-description-title';
		size = { width: TEXT_SIZE_WIDTH, height: objPageSize.height.magnitude * 0.1 };
		offset = { x: OFFSET * 3 + VIDEO_SIZE_WIDTH, y: objPageSize.height.magnitude * 0.1 };
		// strDate
		let objOption = { size, offset, unit: DISTANCE_UNIT, bold: true, italic: false, fontSize: 18 }
		let eleDescriptionTitle = SliderAPI.createTextboxWithText(pid, pageid, strDescriptionTitleId, strCaseDate, objOption);
		// strDescription
		const strDescriptionContentId = pages + '-description-content';
		offset = { x: OFFSET * 3 + VIDEO_SIZE_WIDTH, y: size.height + objPageSize.height.magnitude * 0.1 + 100000 };
		size = { width: TEXT_SIZE_WIDTH, height: objPageSize.height.magnitude - offset.y - objPageSize.height.magnitude * 0.1 };
		objOption = { size, offset, unit: DISTANCE_UNIT, bold: false, italic: true, fontSize: 14 }
		let eleDescriptionContent = SliderAPI.createTextboxWithText(pid, pageid, strDescriptionContentId, strDescription, objOption);
		reqs = [...reqs, ...eleVideo, ...eleDescriptionTitle, ...eleDescriptionContent];
		return reqs;
	}

	editSlides = (result) => {
		var presentationId = result.presentationId;
		const patientInfo = {
			name: this.state.name,
			birthday: this.state.birthdayforComponent,
			bed: this.state.bed,
			phone: this.state.phone,
			email: this.state.email,
			avatar: this.state.thumburl,
			notes: this.state.notes
		}
		//============== subject page ==================
		let reqs = this.addSubjectSlide(result, patientInfo);

		let pages = result.slides.length;
		let req = this.addPatientDescriptionSlide(result, presentationId, pages, patientInfo);
		reqs = [...reqs, ...req];
		if (this.state.chosenPatientImagesList) {
			for (var i = 0; i < this.state.chosenPatientImagesList.length; i++) {
				if ( this.state.chosenPatientImagesList[i].value.media === 'video' ) {
					continue;
					pages = pages + 1;
					req = this.addPatientCaseVideoSlides(presentationId, pages, result.pageSize, this.state.chosenPatientImagesList[i].value);
					
				} else /*if ( this.state.chosenPatientImagesList[i].value.media === 'image' )*/ {
					pages = pages + 1;
					req = this.addPatientCaseImageSlides(presentationId, pages, result.pageSize, this.state.chosenPatientImagesList[i].value);
				}
				reqs = [...reqs, ...req];
			}
		}
		SliderAPI.batchExecute(window.gapi.client.slides.presentations, presentationId, reqs, (response) => {
			this.setState({ bCreatingPresentation: false });
			window.open( 'https://docs.google.com/presentation/d/' + presentationId, '_blank' );
		});
	}

	createPresentation = () => {
		const TITLE = this.state.bed + '-' + this.state.name;
		window.gapi.client.slides.presentations.create({
			title: TITLE
		}).then((response) => {
			this.editSlides(response.result);
		}).catch((err) => {
			console.log(err);
			this.setState({ bCreatingPresentation: false });
		});
	...
	loadClientWhenGapiReady = (scriptEle) => {
		if (scriptEle.getAttribute('gapi_processed')) {
			console.log('Client is ready! Now We can access window.gapi.');
			const CLIENT_ID = '534302772599-h9ms28hq8u9kurnuikg9i2jrrl6d9gku.apps.googleusercontent.com'; // CLIENT SECRET KEY
			const API_KEY = 'AIzaSyAxuUTIK3bSLW9imRewJNJa0k7Y_kw5Lr4';
			...
			window.gapi.load('client:auth2', () => {
				if (!window.gapi || !window.gapi.client) {
					console.log('some errors occured during load');
					this.setState({ bCreatingPresentation: false });
					return;
				}
				...
				window.gapi.client.init({
					...
					console.log('google api client initialization succeeded');
					// console.log(window.gapi.auth2);
					window.gapi.auth2.getAuthInstance().isSignedIn.listen(this.updateSigninStatus);
					let authObj = window.gapi.auth2.getAuthInstance();
					if (!authObj.isSignedIn.get()) {
						firebase.auth().signOut();
						authObj.signIn().then(_ => {
							this.updateSigninStatus(window.gapi.auth2.getAuthInstance().isSignedIn.get());
						}).catch((error) => {
							// in this part you have to show small alert to users instruction
							this.setState({ presentationCreattingInstructionDlg: true, bCreatingPresentation: false });
							console.log('some errors occured during google auth');
							console.log(error);
							firebase.auth().signOut();
						});
					} else {
					...
					console.log('some errors occured during auth');
					console.log(error);
					this.setState({ bCreatingPresentation: false });
				})
			});
			...
			setTimeout(() => { this.loadClientWhenGapiReady(scriptEle) }, 100);
		}
	}

	onCreatePresentation() {
		this.setState({ bCreatingPresentation: true });
		let scriptEle = document.querySelector("script[src='https://apis.google.com/js/client.js']");
		if (scriptEle) {
			this.loadClientWhenGapiReady(scriptEle);
		} else {
	...
	handlePresentationCreattingOK = () => {
		this.setState({ presentationCreattingInstructionDlg: false });
	}

	render() {
		var { classes, theme, isMobile } = this.props;
		...
		var { chosenPatientDiagnosticsList, chosenPatientTagsList, chosenPatientImagesList, chosableDiagnosticsList, chosableTagsList,
			...
			disabledEditingStatue, editButtonLabel, thumburl, uploadingImageType, uploadingMedia, savingDetailCaseImage, presentationSettingDlg, presentationCreattingDlg, presentationCreattingInstructionDlg, presentationSettinginstructionDlg,
			draggingFilesToUplaod, droppedFilesToUpload, uploadingFileList, activeStep, currentDroppedOneImage, detailCaseImageCropping, bCreatingPresentation } = this.state;
		...
		return (
			<Grid container spacing={24} className={classes.eachPatientViewMainContainer}>
				<Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
					<Tabs className={classes.tabs} value={this.state.galleryTabIndex} onChange={this.handleGalleryTabChange} indicatorColor="secondary" textColor="secondary" fullWidth centered>
						<Tab className={classes.eachTab} label="Profile" icon={<PersonPinIcon />} />
						{ isMobile && <Tab className={classes.eachTab} label="Diagnostics" icon={<img alt="Diagnostics" src={DiagnosticsImg} style={{ width: 24, height: 24 }} />} /> }
						{ isMobile && <Tab className={classes.eachTab} label="Tags" icon={<img alt="Tags" src={TagsImg} style={{ width: 24, height: 24 }} />} /> }
						{ !isMobile && <Tab className={classes.eachTab} label="Diagnostics & Tags" icon={<img alt="Diagnostics And Tags" src={DiagnosticsImg} style={{ width: 24, height: 24 }} />} /> }
					...
					{isMobile && <SwipeableViews className={classes.swipeableView} axis={theme.direction === 'rtl' ? 'x-reverse' : 'x'} index={this.state.galleryTabIndex} onChangeIndex={this.handleGalleryTabIndexChange} >
						<TabContainer dir={theme.direction}>
							<Card>
								<CardContent style={{ position: 'relative' }}>
									{bCreatingPresentation && <LinearProgress color="secondary" style={{marginBottom: 20}} />}
									<div className={classes.googlePresentation} onClick={this.handleMakeGooglePresentation}>
							...
							<Card>
								<CardContent className={classes.caseImagesContainer}>
									...
									<div style={{display: 'flex'}}>
										<Fab id="add-media-button" variant="extended" aria-label="AddImage" className={classes.rightFab} color="primary" onClick={this.handleTryToUploadCaseImage} style={{flex: 'auto'}}>
											<AddIcon className={classes.extendedIcon} /> Click or Drop Images&nbsp;/&nbsp;Video to add
										</Fab>
										<div className={classes.googlePresentationInGallery} onClick={this.handleMakeGooglePresentation}>
											<Tooltip placement="top" title={'Make a Presentation'}>
												<img src={PresentationImg} />
											</Tooltip>
										</div>
				...
				<Dialog open={presentationCreattingInstructionDlg} onClose={() => { this.setState({ presentationCreattingInstructionDlg: false }) }} aria-labelledby="presentation-create-dlg" >
					<DialogTitle id="presentation-create-title" className={classes.detailDialogTitle}>
						...
						you have to enable popup in order to run this application properly"}
					</DialogTitle>
					<DialogActions>
						<Button onClick={this.handlePresentationCreattingOK} color="secondary" className={classes.editButton}> OK </Button>
					</DialogActions>
				</Dialog>
				{/* iron-tiger-end */}
			</Grid>
